<div align="center">

# Saikat Bank
### Cloud / DevOps Engineer

Building and automating cloud infrastructure — Kubernetes, IaC, and CI/CD, end to end.

</div>

<br>

## About

3+ years working across cloud infrastructure, container orchestration, and delivery pipelines.
Currently at **SproutsAI**. Day to day, that means designing infrastructure as code, running and
operating Kubernetes clusters, and building CI/CD pipelines that make shipping and rollback
boring — the way it should be.

<br>

## Tech Stack

<table>
<tr>
<td valign="top" width="50%">

**Cloud**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![GCP](https://img.shields.io/badge/Google_Cloud-4285F4?style=flat&logo=googlecloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)

**Container Orchestration**

![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)

</td>
<td valign="top" width="50%">

**Infrastructure as Code**

![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat&logo=terraform&logoColor=white)

**CI/CD**

![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat&logo=jenkins&logoColor=white)

</td>
</tr>
<tr>
<td valign="top" colspan="2">

**Programming & Scripting**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnubash&logoColor=white)

</td>
</tr>
</table>

<br>

## GitHub Activity

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=saikatbank&show_icons=true&hide_title=true&hide_border=true&count_private=true" alt="GitHub stats" height="165">
<img src="https://streak-stats.demolab.com/?user=saikatbank&hide_border=true" alt="GitHub streak stats" height="165">

</div>

Consistently building, automating, and contributing — commit history and streak above reflect
live GitHub data.

<br>

## Engineering Focus

Currently exploring: `<add what you're learning/exploring right now>`

<br>

<div align="center">

## Connect

| | |
|---|---|
| LinkedIn | `<add LinkedIn URL>` |
| Email | saikatbank101@gmail.com |
| Website | `<add website/portfolio URL>` |

</div>
